				...
				struct nvgpu_semaphore_sea *sea)
{
	...
	p->cpu_va = sea->sea_mem.cpu_va + PAGE_SIZE * page_idx; // use va, not page
			     ...
			     struct vm_gk20a *vm)
{
	...
	p->page = vmalloc_to_page(p->cpu_va); // get pages from va
	err = sg_alloc_table_from_pages(p->rw_sg_table, &p->page, 1, 0,
					PAGE_SIZE, GFP_KERNEL);
	//err = sg_alloc_table(p->rw_sg_table, 1, GFP_KERNEL);
	if (err) {
		err = -ENOMEM;
		goto fail;
	}
