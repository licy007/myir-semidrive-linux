...
static int gk20a_vidmem_clear_all(struct gk20a *g)
{
	...
	if (gk20a_fence_out) {
		...
		if (err) {
			gk20a_err(g->dev,
				"fence wait failed for CE execute ops %d", err);
		}
	}
				...
				struct vm_gk20a_mapping_batch *batch)
{
	...
	if (mapped_buffer->flags & NVGPU_AS_MAP_BUFFER_FLAGS_FIXED_OFFSET) {
		...
		while (retries >= 0 || !tegra_platform_is_silicon()) {
			if (atomic_read(&mapped_buffer->ref.refcount.refs) == 1)
				break;
			retries--;
			udelay(5);
		}
...
int gk20a_vidmem_buf_alloc(struct gk20a *g, size_t bytes)
{
	...
	fd = __alloc_fd(current->files, 1024, 1024*1024/*sysctl_nr_open*/, O_RDWR);
	if (fd < 0) {
...
int gk20a_gmmu_alloc_attr(struct gk20a *g, unsigned long attr, size_t size,
		struct mem_desc *mem)
{
...
int gk20a_gmmu_alloc_attr_sys(struct gk20a *g, unsigned long attr,
...
#else
                if (0){//attr == DMA_ATTR_NO_KERNEL_MAPPING) {
                        mem->pages = dma_alloc_attrs(d,
                                        size, &iova, GFP_KERNEL, attr);
                        if (!mem->pages)
                                return -ENOMEM;
                } else {
                        mem->cpu_va = dma_alloc_attrs(d,
...
static void gk20a_gmmu_free_attr_sys(struct gk20a *g, unsigned long attr,
			  struct mem_desc *mem)
{
	struct device *d = dev_from_gk20a(g);
	...
	if (mem->cpu_va || mem->pages) {
		...
		if (0) { // ignore all attrs
			...
			dma_addr_t iova = sg_dma_address(mem->sgt->sgl);
			phys_addr_t paddr = dma_to_phys(d, iova);
...
static int gk20a_gmmu_clear_vidmem_mem(struct gk20a *g, struct mem_desc *mem)
{
	...
	/* if no msi, just release sema here */
	if (!g->msi_enabled)
		gk20a_channel_semaphore_wakeup(g, false); // disable msi
...
int gk20a_gmmu_alloc_attr_vid(struct gk20a *g, unsigned long attr,
		size_t size, struct mem_desc *mem)
{
...
int gk20a_gmmu_alloc_attr_vid_at(struct gk20a *g, unsigned long attr,
...
static void gk20a_gmmu_free_attr_vid(struct gk20a *g, unsigned long attr,
			  struct mem_desc *mem)
{
#if defined(CONFIG_GK20A_VIDMEM)
	bool was_empty;
...
void gk20a_gmmu_free_attr(struct gk20a *g, unsigned long attr,
			  struct mem_desc *mem)
{
	switch (mem->aperture) {
...
static struct mem_desc *get_pending_mem_desc(struct mm_gk20a *mm)
{
	...
	if (mem == NULL)
	    gk20a_dbg_info("pending is (null)\n");
...
int gk20a_gmmu_alloc_map_attr(struct vm_gk20a *vm,
			 unsigned long attr, size_t size, struct mem_desc *mem)
{
		...
		char *name)
{
	...
	pr_debug("Init space for %s: va_limit=0x%llx, "
	...
	pr_debug("user_vma     [0x%llx,0x%llx)",
		       user_vma_start, user_vma_limit);
	pr_debug("user_lp_vma  [0x%llx,0x%llx)",
		       user_lp_vma_start, user_lp_vma_limit);
	pr_debug("kernel_vma   [0x%llx,0x%llx)",
