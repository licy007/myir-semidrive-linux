...
int dw_pcie_host_init(struct pcie_port *pp)
{
	...
	resource_list_for_each_entry_safe(win, tmp, &bridge->windows) {
		...
		case IORESOURCE_IO:
			...
			break;
		case IORESOURCE_MEM:
			if ((win->res->flags & IORESOURCE_PREFETCH ) == IORESOURCE_PREFETCH) {
				pp->prefmem = win->res;
				pp->prefmem->name = "PREFMEM";
				pp->prefmem_size = resource_size(pp->prefmem);
				pp->prefmem_bus_addr = pp->prefmem->start - win->offset;
			} else {
				pp->mem = win->res;
				pp->mem->name = "MEM";
...
void dw_pcie_setup_rc(struct pcie_port *pp)
{
	...
	if (!pp->ops->rd_other_conf) {
		...
		if (pp->prefmem) {
			pp->prefmem_base = pp->prefmem->start;
			dw_pcie_prog_outbound_atu(pci, PCIE_ATU_REGION_INDEX3,
					  PCIE_ATU_TYPE_MEM, pp->prefmem_base,
					  pp->prefmem_bus_addr, pp->prefmem_size);
		}
