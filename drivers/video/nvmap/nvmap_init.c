					...
					struct device *dev)
{
	...
	struct cma *cma = (struct cma*)(co->cma_dev);
	int err;
...
#if 0 // add by bin
	if (!co->cma_dev) {
		//co->dma_dev = dev;// set to carveout add by xx
		err = dma_declare_coherent_memory(co->dma_dev, 0,
		//err = dma_declare_coherent_memory(co->dma_dev, co->base,
				co->base, co->size,
...
int __init nvmap_co_setup(struct reserved_mem *rmem)
{
	...
	//dev_set_cma_area(co->cma_dev, cma); // del by bin
	co->cma_dev = (struct device*)(cma); // add by bin save cma
	pr_info("tegra-carveouts carveout=%s %pa@%pa\n",
