...
#endif
	{
		va = (phys_addr_t)dma_alloc_attrs(dev, len, &pa, GFP_KERNEL, 0);
		dev_dbg(dev, "dma alloc get pa:%pa, va:%pa\n", &pa, &va);
		if (!dma_mapping_error(dev, pa))
			dev_dbg(dev, "Allocated addr (%pa) len(%zu)\n",	&pa, len);
		else
			dev_err(dev, "dma mapping error failed at pa:%pa\n", &pa);
	}
...
static void nvmap_free_mem(struct nvmap_heap *h, phys_addr_t base,
				size_t len, phys_addr_t cpu_va)
{
	struct device *dev = h->cma_dev; // use cma
					      ...
					      phys_addr_t *start)
{
	...
	dev_base = nvmap_alloc_mem(heap, len, &cpu_va);
#if 0 // already va
	...
	//heap_block->block.base = (phys_addr_t)(phys_to_virt(dev_base));
	//heap_block->orig_addr = dev_base;
	heap_block->orig_addr = cpu_va; // record va
	heap_block->size = len;
...
static struct list_block *do_heap_free(struct nvmap_heap_block *block)
{
	...
	nvmap_free_mem(heap, block->base, b->size, b->orig_addr);
				     ...
				     phys_addr_t base, size_t len, void *arg)
{
	...
	if (co->cma_dev) {
		...
		//err = dma_declare_coherent_memory(h->dma_dev, base, base, len,
