...
#define RPMSG_PROP_ADDR (127)
#define RPMSG_PROP_NAME "prop-ap"
			     ...
			     struct rpmsg_hdr *msg, unsigned int len)
{
	...
	if (ept) {
		...
		dev_warn_ratelimited(dev, "msg (%d->%d) received with no recipient\n",
					msg->src, msg->dst);
		       ...
		       void *priv, u32 src)
{
	...
	if ((strcmp(RPMSG_PROP_NAME, chinfo.name) == 0)
		 && (chinfo.dst == RPMSG_PROP_ADDR)) {
		chinfo.src = RPMSG_PROP_ADDR;
	}
...
#ifdef CONFIG_PM
static int rpmsg_restore(struct virtio_device *dev)
{
	struct virtproc_info *vrp = dev->priv;
	virtqueue_notify(vrp->rvq);
	return 0;
}
#endif
