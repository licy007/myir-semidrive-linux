...
int rpmsg_eptdev_get_property(struct rpmsg_eptdev *eptdev, unsigned int cmd, unsigned long arg)
{
	struct rpmsg_endpoint_property data;

	memset(&data, 0, sizeof(data));
	data.src = eptdev->chinfo.src;
	data.dst = eptdev->chinfo.dst;
	data.mtu = rpmsg_get_mtu(eptdev->ept);
	strncpy(data.name, eptdev->chinfo.name, RPMSG_NAME_SIZE);
			       ...
			       unsigned long arg)
{
	...
	case RPMSG_GETPROP_EPT_IOCTL:
	ret = rpmsg_eptdev_get_property(eptdev, cmd, arg);
		break;
