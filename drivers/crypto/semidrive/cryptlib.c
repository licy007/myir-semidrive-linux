...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 8, 0))
		tfm = cryptodev_crypto_blkcipher_tfm(out->async.s);
								 ...
								 size_t len)
{
	...
	struct semidrive_cipher_ctx *ctx = NULL;
	struct crypto_ablkcipher **sk_tfm_ctx;
	...
	if (cdata->aead == 0) {
		cryptodev_blkcipher_request_set_crypt(cdata->async.request,
											  ...
											  len, cdata->async.iv);
		skcipher = crypto_skcipher_reqtfm(cdata->async.request);
		sk_tfm_ctx = crypto_skcipher_ctx(skcipher);
		ablkcipher = *sk_tfm_ctx;
		ablk_tfm = crypto_ablkcipher_tfm(ablkcipher);
		ctx = crypto_tfm_ctx(ablk_tfm);
	...
	res = waitfor(&cdata->async.result, ret);

	if (ctx != NULL) {
		ctx->aes_ctx = CTX_MIDDLE;
	}
...
int cryptodev_hash_copy(struct hash_data *dst, struct hash_data *src)
{
	...
	reqsize = crypto_ahash_reqsize(src->async.s);

	if (unlikely(reqsize <= 0)) {
		return -EINVAL;
	}

	reqdata = kzalloc(reqsize, GFP_KERNEL);

	if (unlikely(reqdata == NULL)) {
		return -ENOMEM;
	}

	ret = crypto_ahash_export(src->async.request, reqdata);
	...
	ret = crypto_ahash_import(dst->async.request, reqdata);
