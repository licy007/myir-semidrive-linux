...
static enum hrtimer_restart txdone_hrtimer(struct hrtimer *hrtimer)
{
	...
	if (resched || mbox->hrt_state == HRT_RESCHEDULE) {
		...
		hrtimer_forward_now(hrtimer, ns_to_ktime(mbox->txpoll_period));
...
static void mbox_cancel_message(struct mbox_chan *chan)
{
	if (chan->mbox->ops->cancel_data)
		return chan->mbox->ops->cancel_data(chan);
}
...
int mbox_send_message(struct mbox_chan *chan, void *mssg)
{
	...
	if (t < 0) {
		dev_err(chan->cl->dev, "tx queue full(%d), may broken link\n",
				MBOX_TX_QUEUE_LEN);
		mbox_cancel_message(chan);
		return t;
	}
