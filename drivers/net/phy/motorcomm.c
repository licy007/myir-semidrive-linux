...
static int yt8521_config_init(struct phy_device *phydev)
{
	...
	if (ret < 0)
		return ret;
	/*enable led1  led2*/
	ret = ytphy_write_ext(phydev, 0xa00d, 0x1e00);//led1
	ret = ytphy_write_ext(phydev, 0xa00e, 0x1800);//led2
...
static int yt8521_soft_reset(struct phy_device *phydev)
{
	...
	pr_err("eth yt8521_soft_reset start \n");
	// switch to addr 0xa000
	ret = phy_write(phydev, 0x1e, 0xa000);
	if (ret < 0)
		return ret;
...
static int yt8521_1G_rxtx_delay(struct phy_device *phydev, int speed)
{
	int ret;
	int val;
		/* enable RX delay */
	...
	if (speed == 1000)
		...
		val = 0xf1;
	ret =  ytphy_write_ext(phydev, 0xa003, (val | 0x3));
		if (ret < 0)
			return ret;
	return ret;
}
...
static int yt8521_read_status(struct phy_device *phydev)
{
	...
	bool need_try_1G_100;

	if (phydev->link)
		need_try_1G_100 = false;
	else
		need_try_1G_100 = true;
	...
	if (link_utp || link_fiber) {
		...
		if (need_try_1G_100) {
			...
			if (phydev->speed == SPEED_1000)
				ret = yt8521_1G_rxtx_delay(phydev, 1000);
			else
				ret = yt8521_1G_rxtx_delay(phydev, 100);
		}
	} else {
		phydev->link = 0;
		need_try_1G_100 = true;
