 ...
 *
 * Contents:  network interface functions as called from Linux kernel
 *
 * Path  : OAK-Linux::OAK Switch Board - Linux Driver::Class Model::oak_net
 * Author: afischer
 * Date  :2020-05-13  - 16:57
 *  */
#include "oak_net.h"
...
int oak_net_esu_set_mtu(struct net_device* net_dev, int new_mtu)
{
    ...
    /* UserCode{2615370D-816B-4630-A294-8DE751729872}:tDUl6BNW0Y */
    data = sr32(np, offs);
    data &= ~(3 << 12);
    /* UserCode{2615370D-816B-4630-A294-8DE751729872} */
    ...
    if (fs > 1522)
    {
        ...
        if (fs <= 2048)
        {
            /* UserCode{BA43564A-851B-4077-9BEF-7BBE07730E81}:1JHxzimhdX */
            data |= (1 << 12);
            /* UserCode{BA43564A-851B-4077-9BEF-7BBE07730E81} */
        }
        else
        {
            /* UserCode{D8BD8A29-B52D-4d01-84AA-9F746A54242A}:1DERkBQy2Q */
            data |= (2 << 12);
            /* UserCode{D8BD8A29-B52D-4d01-84AA-9F746A54242A} */
        }
    }
    else
    {
    }
    /* UserCode{DA6A132D-B95F-463c-8BB6-2FCC121E1F42}:1f1SN5lR3J */
    oakdbg(debug, PROBE, "MTU %d/%d data=0x%x", new_mtu, fs, data);
    /* UserCode{DA6A132D-B95F-463c-8BB6-2FCC121E1F42} */

    /* UserCode{CC4F5040-B83A-49d0-AE21-6E30D7E1176E}:ya0FogcAyq */
    net_dev->mtu = new_mtu;
    /* UserCode{CC4F5040-B83A-49d0-AE21-6E30D7E1176E} */

    /* UserCode{6D82062B-B7B1-476a-9ED4-1456926BCA4F}:19WRvjU3yH */
    sw32(np, offs, data);
    /* UserCode{6D82062B-B7B1-476a-9ED4-1456926BCA4F} */
...
int oak_net_open(struct net_device* net_dev)
{
    ...
    int err_23; /* start of activity code */
            ...
            if (err == 0)
            {
                ...
                if (err == 0)
                {
                    ...
                    if (err == 0)
                    {
                        ...
                        err = oak_net_esu_set_mtu(net_dev, np->netdev->mtu);
...
{
    oak_t* np = netdev_priv(net_dev); /* automatically added for object flow handling */
    int rc_19 = -EOPNOTSUPP; /* start of activity code */

    if (cmd == OAK_IOCTL_REG_MAC_REQ || cmd == OAK_IOCTL_REG_ESU_REQ)
    {
        rc_19 = oak_ctl_direct_register_access(np, ifr, cmd);
    }
    else
    {
    }
    ...
    {
        rc_19 = oak_ctl_load_generator_access(np, ifr, cmd);
    }
    else
    {
    }
    ...
    {
        rc_19 = oak_ctl_channel_status_access(np, ifr, cmd);
    }
    else
    {
    }
    ...
    {
        rc_19 = oak_ctl_set_mac_rate(np, ifr, cmd);
    }
    else
    {
    }
    ...
    {
        rc_19 = oak_ctl_set_rx_flow(np, ifr, cmd);
    }
    else
    {
    }
...
/* Name      : select_queue
 * Returns   : uint16_t
 * Parameters:  struct net_device * dev = dev,  struct sk_buff * skb = skb,  void * priv = priv,  select_queue_fallback_t cb = cb
 *  */
uint16_t oak_net_select_queue(struct net_device* dev, struct sk_buff* skb, void* priv, select_queue_fallback_t cb)
{
    oak_t* np = netdev_priv(dev);
    uint32_t txq = 0;
...
static int oak_net_tx_packet(oak_t* np, struct sk_buff* skb, uint16_t txq)
{
	 ...
	 * So zero byte transfer should not be
	 * programmed in the descriptor. */
	if (skb->len < OAK_ONEBYTE)
        ...
        else
        {
            ...
            if (nfrags > 0)
            {
                /* UserCode{43DFC20D-D464-4d5d-90B9-19BCA9C989E7}:17ZXFkUAH6 */
                frag = &skb_shinfo(skb)->frags[frag_idx];
                len = skb_frag_size(frag);
                mapping = dma_map_page(np->device, skb_frag_page(frag), frag->page_offset, len, DMA_TO_DEVICE);
        ...
        if (num > 0)
        {
            ...
            while (frag_idx < nfrags)
            {
                ...
                /* UserCode{25537965-6577-4f39-A397-4A8A32EE61E7}:AsRjOMAzz1 */
                mapping = dma_map_page(np->device, skb_frag_page(frag), frag->page_offset, len, DMA_TO_DEVICE);
...
static int oak_net_tx_work(ldg_t* ldg, uint32_t ring, int budget)
{
    ...
    int rc_14; /* start of activity code */
...
/* UserCode{0DC6EF08-D642-4571-902D-D7DDE07E74C5}:cR92LBeUkf */
    ...
    if (txc->tbr_len > 0)
    {
        ...
        if ((txc->flags & OAK_RX_LGEN_TX_MODE) == OAK_RX_LGEN_TX_MODE)
        {
            work_done = oak_ctl_process_tx_pkt_lgen(np, ring, todo);
        }
        else
        {
...
static int oak_net_rx_work(ldg_t* ldg, uint32_t ring, int budget)
{
    ...
    int rc_16; /* start of activity code */
...
/* UserCode{16314771-E1E1-4091-98B7-2CFE72B6C620}:yDFj0r5wf2 */
    ...
    while ((todo > 0) && (rxc->rbr_len > 0))
    {
        ...
        if ((rxc->flags & OAK_RX_LGEN_RX_MODE) == OAK_RX_LGEN_RX_MODE)
        {
            compl = oak_ctl_lgen_rx_pkt(np, ring, rxc->rbr_len, &skb);
        }
        else
        {
            ...
            if (page != NULL)
            {
                ...
                if (good_frame == 1)
                {
                    ...
                    if (mhdr != 0)
                    {
                        ...
                        {
                            /* UserCode{F2B075AC-4174-4b7c-A264-1D4DC827F172}:ZRcCudxfG4 */
                            offs = 0;
                            /* UserCode{F2B075AC-4174-4b7c-A264-1D4DC827F172} */
                        }
                    }
                    else
                    {
                    ...
                    dma_sync_single_for_cpu(np->device, page_to_phys(page)+ rba->page_offs + offs, blen,DMA_FROM_DEVICE);
