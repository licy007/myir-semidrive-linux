...
np->gicu.ldg[grp].msi_ge = (val << 32ULL);
    ...
    if (err == 0)
    {
        ...
        while ((i < np->gicu.num_ldg) && (err == 0))
        {
            ...
            if (val != 0)
            {
                err = oak_irq_request_single_ivec(np, p, val, i);
...
int oak_irq_request_single_ivec(struct oak_tStruct* np, ldg_t* ldg, uint64_t val, uint32_t idx)
{
    ...
    err = request_irq(np->gicu.msi_vec[idx].vector, oak_irq_callback, 0, np->gicu.ldg[idx].msiname, &np->gicu.ldg[idx]);

    /* UserCode{AD709810-0D5D-4f5b-8505-9A44C38D0210} */
...
void oak_irq_release_ivec(struct oak_tStruct* np)
{
    ...
    while (i < np->gicu.num_ldg)
    {
        ...
        {
            /* UserCode{E3C81644-90E9-4722-B823-4B9C7CA6E4CD}:1amzhKxakJ */
