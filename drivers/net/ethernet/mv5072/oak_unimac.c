 ...
 *
 * Path  : OAK-Linux::OAK Switch Board - Linux Driver::Class Model::oak_unimac
 * Author: afischer
 * Date  :2020-09-10  - 12:04
 *  */
...
int oak_unimac_alloc_channels(oak_t* np, int rxs, int txs, int chan, int rto)
{
    ...
    while (err == 0 && i < max_channel)
    {
        ...
        if (rxc->rbr_bpage < 1)
        {
            /* UserCode{E3887D53-5BB3-413a-99CE-C0AD78D63DB6}:e88et8mSQf */
            err = -EFAULT;
            /* UserCode{E3887D53-5BB3-413a-99CE-C0AD78D63DB6} */
        }
        else
        {
        }
    ...
    if (ring >= 0 && ring <= 9)
    {
        ...
        if (np->pci_class_revision >= 1)
        {
            ...
            if (reg == OAK_UNI_DMA_TX_CH_CFG)
            {
                ...
                {
                    val = oak_unimac_io_read_32(np, reg);
                    /* UserCode{6073D3C9-22EB-4ff4-A038-151C1E90E02D}:t9Ns8TMrWt */
                    val &= ~0x7F; /* Max burst size */
                    val |= ((np->rrs / 8) - 1) & 0x7F;
                    /* UserCode{6073D3C9-22EB-4ff4-A038-151C1E90E02D} */
                    ...
                    oak_unimac_io_write_32(np, reg, val);
                    /* UserCode{A8BBACF4-F27C-48a8-8EF1-743DEF4C4A65}:1d6xMkzvo0 */
                    oakdbg(debug, DRV, "TX max burst size: %d, val=0x%x", np->rrs, val);

                    /* UserCode{A8BBACF4-F27C-48a8-8EF1-743DEF4C4A65} */
                }
                else
                {
                }
