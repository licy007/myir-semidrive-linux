...
static int stmmac_dwmac5_quirks(struct stmmac_priv *priv)
{
	if (priv->plat->force_disable_asp)
		priv->dma_cap.asp = 0;
	if(priv->plat->multicast_filter_regs)
		priv->hw->multicast_filter_bins = priv->plat->multicast_filter_regs;
	if(priv->plat->mcast_bits_log2)
		priv->hw->mcast_bits_log2 = priv->plat->mcast_bits_log2;
	return 0;
}
