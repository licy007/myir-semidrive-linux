...
int msg_buf_size = BRCMF_MSGBUF_MAX_PKT_SIZE;
...
static u32 brcmf_msgbuf_rxbuf_data_post(struct brcmf_msgbuf *msgbuf, u32 count)
{
	...
	for (i = 0; i < alloced; i++) {
		...
		skb = brcmu_pkt_buf_get_skb(msg_buf_size);
...
bool brcmf_set_msg_buffer_size(void) {
	struct device_node *new_np;
	bool ret = false;
	// msg_buf_size = BRCMF_MSGBUF_MAX_PKT_SIZE;
	new_np = of_find_compatible_node(NULL, NULL, "semidrive,kunlun-pcie");
	if (!new_np) return ret;

	if (of_find_property(new_np, "use-small-buff", NULL))
		msg_buf_size = 4096;

	brcmf_err("Msg buffer set to %d\n", msg_buf_size);
	ret = true;
