...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	 ...
	 */
	dhd_update_fw_nv_path(dhd);
#ifdef CONFIG_SET_FW_NV_CLM_BY_CHIP
	dhd_set_fw_nv_clm_by_chip(dhd, bus);
#endif
...
#ifndef CONFIG_FW_NV_CLM_PATH
#define CONFIG_FW_NV_CLM_PATH "/etc/firmware/wlan/"
#endif
#define FW_NAME_BCM89373_PCIE "fw_8x373_pcie.bin"
#define NV_NAME_BCM89373_PCIE "nv_8x373_pcie.txt"
#define CLM_NAME_BCM89373_PCIE "clm_8x373_pcie.blob"

#define FW_NAME_BCM89459_PCIE "89459-pcie.bin"
#define NV_NAME_BCM89459_PCIE "89459_nvram.txt"
#define CLM_NAME_BCM89459_PCIE "89459-pcie.clm_blob"

#define FW_NAME_BCM89570_PCIE "fw_8x570_pcie.trxse"
#define NV_NAME_BCM89570_PCIE "nv_8x570_pcie.txt"
#define CLM_NAME_BCM89570_PCIE "clm_8x570_pcie.blob"

char def_clm_path[MOD_PARAM_PATHLEN];

void dhd_set_fw_nv_clm_by_chip(struct dhd_info *dhdinfo, struct dhd_bus *bus)
{
	uint chip, chiprev;
	char *fw, *nv, *clm, *path;
	uint fw_nlen, nv_nlen, clm_nlen, path_len;

	/* If dhdinfo->fw_path initiated from firmware_path and nvram_path, skip */
	if ( (dhdinfo->fw_path[0] != '\0') && (dhdinfo->nv_path[0] != '\0') ) {
		DHD_ERROR(("%s Using firmware_path and nvram_path, skip set fw&nv&clm by chip\n", __FUNCTION__));
		return;
	}
	
	chip = dhd_bus_chip(bus);
	chiprev = dhd_bus_chiprev(bus);
	path = CONFIG_FW_NV_CLM_PATH;
	path_len = strlen(CONFIG_FW_NV_CLM_PATH);	
	fw = nv = clm = NULL;
	fw_nlen = nv_nlen = clm_nlen = 0;

	DHD_ERROR(("%s chip:0x%x, chiprev:%d, path:%s\n", __FUNCTION__, chip, chiprev, path));
	
	switch (chip) {
		case BCM4355_CHIP_ID:
			fw = FW_NAME_BCM89459_PCIE;
			fw_nlen = strlen(FW_NAME_BCM89459_PCIE);
			nv = NV_NAME_BCM89459_PCIE;
			nv_nlen = strlen(NV_NAME_BCM89459_PCIE);
			clm = CLM_NAME_BCM89459_PCIE;
			clm_nlen = strlen(CLM_NAME_BCM89459_PCIE);
			break;
		case BCM4373_CHIP_ID:
			fw = FW_NAME_BCM89373_PCIE;
			fw_nlen = strlen(FW_NAME_BCM89373_PCIE);
			nv = NV_NAME_BCM89373_PCIE;
			nv_nlen = strlen(NV_NAME_BCM89373_PCIE);
			clm = CLM_NAME_BCM89373_PCIE;
			clm_nlen = strlen(CLM_NAME_BCM89373_PCIE);
			
		case CYW55560_CHIP_ID:
		//case CYW55560_WLAN_ID:
		//case CYW89570_WLAN_ID:
			fw = FW_NAME_BCM89570_PCIE;
			fw_nlen = strlen(FW_NAME_BCM89570_PCIE);
			nv = NV_NAME_BCM89570_PCIE;
			nv_nlen = strlen(NV_NAME_BCM89570_PCIE);
			clm = CLM_NAME_BCM89570_PCIE;
			clm_nlen = strlen(CLM_NAME_BCM89570_PCIE);
	...
	if (fw && fw[0] != '\0') {
		strncpy(dhdinfo->fw_path, path, path_len);
		strncpy(dhdinfo->fw_path+path_len, fw, fw_nlen);
		dhdinfo->fw_path[path_len+fw_nlen+1] = '\0';
		DHD_ERROR(("%s dhdinfo->fw_path: %s\n", __FUNCTION__, dhdinfo->fw_path));
	}
	if (nv && nv[0] != '\0') {      
		strncpy(dhdinfo->nv_path, path, path_len);
		strncpy(dhdinfo->nv_path+path_len, nv, nv_nlen);
		dhdinfo->nv_path[path_len+nv_nlen+1] = '\0';
		DHD_ERROR(("%s dhdinfo->nv_path: %s\n", __FUNCTION__, dhdinfo->nv_path));
	}
	if( clm && clm[0] != '\0') {  
		strncpy(def_clm_path, path, path_len);
		strncpy(def_clm_path + path_len, clm, clm_nlen);
		def_clm_path[path_len + clm_nlen + 1] = '\0';
		strcpy(clm_path, def_clm_path);
		DHD_ERROR(("%s clm_path: %s\n", __FUNCTION__, clm_path));
	}
