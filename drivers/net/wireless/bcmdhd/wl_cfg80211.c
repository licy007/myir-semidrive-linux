...
static const rsn_cipher_algo_entry_t rsn_cipher_algo_lookup_tbl[] = {
	...
	{WLAN_DHD_CIPHER_SUITE_SMS4, SMS4_ENABLED, CRYPTO_ALGO_SMS4},
#endif /* BCMWAPI_WPI */
#ifdef WL_GCMP
...
wl_set_wpa_version(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	} else if (sme->crypto.n_ciphers_pairwise &&
		is_wapi(sme->crypto.ciphers_pairwise[0])) {
		WL_DBG((" * wl_set_wpa_version, set wpa_auth"
			" to WPA_AUTH_WAPI 0x400 ciphers_pairwise[0]:0x%x", sme->crypto.ciphers_pairwise[0]));
		val = WAPI_AUTH_PSK | WAPI_AUTH_UNSPECIFIED;
	}
#endif // endif
...
wl_set_set_cipher(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	if (is_wps_conn(sme)) {
		...
		if (is_wapi(sme->crypto.cipher_group)) {
	...
	struct cfg80211_connect_params *sme)
{
	...
	//else {
	if (sme->crypto.wpa_versions & NL80211_WAPI_VERSION_1 || (sme->crypto.n_ciphers_pairwise && (is_wapi(sme->crypto.ciphers_pairwise[0])))) {
		WL_ERR(("4. clear auth for WAPI to avoid previous non-open auth(e.g. SAE) side effect.\n"));
	...
	struct cfg80211_ap_settings *info)
{
	...
	if (is_wapi(info->crypto.ciphers_pairwise[0])) {
		WL_ERR(("%s info->crypto.ciphers_pairwise[0] is "
				"WLAN_CIPHER_SUITE_SMS4 \n", __FUNCTION__));
