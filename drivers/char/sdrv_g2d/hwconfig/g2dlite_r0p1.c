...
#define G2DLITE_DUMP_HVSCALER 0
...
#define G2DLITE_MLC_LAYER_COUNT 2
#define G2DLITE_MLC_PATH_COUNT 3
#define G2DLITE_RCHN_COUNT 5

#define G2DLITE_RDMA_APIPE_CH 4
#define G2DLITE_RDMA_WML_TOTAL 64
#define G2DLITE_RDMA_DFIFO_TOTAL_DEPTH 8
#define G2DLITE_RDMA_CFIFO_TOTAL_DEPTH 8

#define G2DLITE_WDMA_WML_TOTAL 64
#define G2DLITE_WDMA_DFIFO_TOTAL_DEPTH 8
#define G2DLITE_WDMA_CFIFO_TOTAL_DEPTH 8
...
static int g2dlite_irq_handler(struct sdrv_g2d *gd)
{
//	return g2dlite_r0p1_irq_handler(gd->iomem_regs);
...
static int g2dlite_reset(struct sdrv_g2d *gd)
{
	//g2dlite_r0p1_sw_reset(gd->iomem_regs);
	g2d_reg_t reg = (g2d_reg_t )gd->iomem_regs;
	...
	G2D_DBG("g2dlite reset done \n");
	return 0;
}

static int g2dlite_enable(struct sdrv_g2d *gd, int enable)
{
	//return g2dlite_r0p1_enable(gd->iomem_regs, &gd->cmd_info[0], gd->write_mode, gd->dma_buf);
	g2d_reg_t reg = (g2d_reg_t )gd->iomem_regs;
...
static int g2dlite_rdma_set(struct sdrv_g2d *gd, struct g2d_input *ins, bool fastcopy)
{
	int i, count = 0;
	char ch[G2DLITE_RCHN_COUNT] = {0};
...
set_rdma_regs:
	wml_unit = G2DLITE_RDMA_WML_TOTAL / count;
	d_depth_unit = G2DLITE_RDMA_DFIFO_TOTAL_DEPTH / count;
	c_depth_unit = G2DLITE_RDMA_CFIFO_TOTAL_DEPTH / count;
	for (i = 0; i < G2DLITE_RCHN_COUNT; i++) {
...
static int g2dlite_wdma_set(struct sdrv_g2d *gd, struct g2d_output_cfg *output, bool fastcopy)
{
	int i, count = 0;
...
set_wdma_regs:
	wml_unit = G2DLITE_WDMA_WML_TOTAL / count;
	d_depth_unit = G2DLITE_WDMA_DFIFO_TOTAL_DEPTH / count;
	c_depth_unit = G2DLITE_WDMA_CFIFO_TOTAL_DEPTH / count;
...
static int g2dlite_rwdma_set(struct sdrv_g2d *gd, struct g2d_input *ins)
{
	int ret;

	ret = g2dlite_rdma_set(gd, ins, false);
	if (ret) {
		G2D_ERROR("g2dlite rdma set failed\n");
		return -1;
	}

	ret = g2dlite_wdma_set(gd, &ins->output, false);
	if (ret) {
		G2D_ERROR("g2dlite wdma set failed\n");
		return -1;
	}
...
static const uint32_t G2DLITE_HSCALER[1][2] = {
    {0x2600, 0x29ff},//{HS_0_SADDR, HS_0_EADDR}
};

static const uint32_t G2DLITE_VSCALER[1][2] = {
...
#if G2DLITE_DUMP_HVSCALER
static void g2dlite_dump_hvscaler(struct sdrv_g2d *gd)
{
	int i;
...
#define GP_HS_CTRL                      0x0300
	...
	for (i = 0; i < 33; i++) {
		G2D_INFO(">>> hscaler coef reg = 0x%lx, val = 0x%x \n",
		 (base + G2DLITE_HSCALER[0][0] + i * 4), g2d_read(base, (G2DLITE_HSCALER[0][0] + i * 4)));
		G2D_INFO(">>> hscaler coef reg = 0x%lx, val = 0x%x \n",
		 (base + G2DLITE_HSCALER[0][0] + 0x100 + i * 4), g2d_read(base, (G2DLITE_HSCALER[0][0] + 0x100 + i * 4)));
	}

	for (i = 0; i < 33; i++) {
		G2D_INFO(">>> vscaler coef reg = 0x%lx, val = 0x%x \n",
		(base + G2DLITE_VSCALER[0][0] + i * 4), g2d_read(base, (G2DLITE_VSCALER[0][0] + i * 4)));
	}
...
static void g2dlite_set_hscaler_coef(struct sdrv_g2d *gd, const int (*hcoef)[5])
{
	int i;
	...
	for (i = 0; i < 33; i++) {
		...
		writel(head, (void *)(base + G2DLITE_HSCALER[0][0] + i * 4));
		writel(tail, (void *)(base + G2DLITE_HSCALER[0][0] + 0x100 + i * 4));
	}
}

static void g2dlite_set_vscaler_coef(struct sdrv_g2d *gd, const int (*vcoef)[4])
{
	unsigned long base = (unsigned long)gd->iomem_regs;
	unsigned int value;
	int i;
	...
	for (i = 0; i < 33; i++) {
		...
		writel(value, (void *)(base + G2DLITE_VSCALER[0][0] + i * 4));
	}
}

static void g2dlite_init_set_scaler_coef(struct sdrv_g2d *gd)
{
	g2dlite_set_hscaler_coef(gd, hscaler_coef);
	g2dlite_set_vscaler_coef(gd, vscaler_coef);
#if G2DLITE_DUMP_HVSCALER
	g2dlite_dump_hvscaler(gd);
#endif
}

static void g2dlite_mlc_init(struct sdrv_g2d *gd)
{
	int i;
	REGISTER mlc_reg *reg = (REGISTER mlc_reg *)(gd->iomem_regs + MLC_JUMP);
	...
	/*LAYER related*/
	for (i = 0; i < G2DLITE_MLC_LAYER_COUNT; i++) {
	...
	/*PATH related*/
	for (i = 0; i < G2DLITE_MLC_PATH_COUNT; i++) {
...
static void g2dlite_rdma_init(struct sdrv_g2d *gd)
{
	int i;
	REGISTER rdma_reg *rdma = (REGISTER rdma_reg *)(gd->iomem_regs + RDMA_JUMP);
...
static void g2dlite_wdma_init(struct sdrv_g2d *gd)
{
...
static void g2dlite_wpipe_init(struct sdrv_g2d *gd)
{
	...
	for (i = 0, n = 0; i < 7; i++) {
		wp->wp_csc_coef[i] = csc_param[3][n] << 0 | csc_param[3][n + 1] << 16;
		n += 2;
	}

	wp->wp_csc_coef[7] = csc_param[3][14] << 0 ;
}

static int g2dlite_init(struct sdrv_g2d *gd)
{
	gd->write_mode = G2D_CMD_WRITE;//G2D_CPU_WRITE;

	g2dlite_init_set_scaler_coef(gd);

	g2dlite_reset(gd);
	g2dlite_rdma_init(gd);
	g2dlite_mlc_init(gd);
	g2dlite_wdma_init(gd);
	g2dlite_wpipe_init(gd);

	if (!gd->du_inited) {
		gd->du_inited = true;
		g2d_choose_pipe(gd, G2D_GPIPE0, TYPE_GP_HIGH, GP0_JUMP);
		g2d_choose_pipe(gd, G2D_SPIPE0, TYPE_SPIPE, SP0_JUMP);
		G2D_DBG("add %d pipes for g2dlite\n", gd->num_pipe);
	}

	return 0;
}

static void g2dlite_mlc_clear(struct sdrv_g2d *gd, int cmdid)
{
	unsigned int val, i;
	cmdfile_info *cmd = (cmdfile_info *)&gd->cmd_info[0];
	/*disable all layers*/
...
static int g2dlite_mlc_set(struct sdrv_g2d *gd, int cmdid, struct g2d_input *input)
{
	unsigned int i, val;
	cmdfile_info *cmd = (cmdfile_info *)&gd->cmd_info[0];
	unsigned int sf_val[G2DLITE_MLC_LAYER_COUNT], path_val[G2DLITE_MLC_PATH_COUNT];
	unsigned char layer_index, path_index;
	struct g2d_layer *layer;

	g2dlite_mlc_clear(gd, cmdid);
	...
	for (i = 0; i < G2DLITE_MLC_LAYER_COUNT; i++) {
		...
		if (layer->index >= G2DLITE_MLC_LAYER_COUNT) {
			return -1;
		}
...
static int g2dlite_wpipe_pixcomp_set(struct sdrv_g2d *gd, struct g2d_output_cfg *output)
{
	int ret = 0;
	unsigned int val;
...
static int g2dlite_wp_csc_set(struct sdrv_g2d *gd, struct g2d_output_cfg *output, unsigned char flag)
{
...
static int g2dlite_wpipe_set(struct sdrv_g2d *gd,
				int cmdid, struct g2d_output_cfg *output)
{
	...
	if (!cmdid) {
		ret = g2dlite_wpipe_pixcomp_set(gd, output);
		if(ret < 0) {
			return ret;
		}
		g2dlite_wp_csc_set(gd, output, G2D_FUN_BLEND);
	}
	...
	//g2dlite_r0p1_wpipe_set(&gd->cmd_info[0], cmdid, output);
 ...
 * 2. Horizontal and vertical scaling at the same time can only be used with G2DLITE GPIPE0
 */
static int g2dlite_check_is_stroke(struct g2d_input *ins)
{
	int i;
	int stroke_action = 0;
	...
	for (i = 0; i < ins->layer_num; i++) {
		...
		if (layer->comp.is_yuv) {
			if (layer->index != G2D_GPIPE0) {
				G2D_ERROR("only G2D_GPIPE0 support yuv format! layer index = %d \n", layer->index);
				return -1;
			}
		}
		...
		/*HScaling & Vscaler can only use GPIPE0*/
		if ((layer->src_h != layer->dst_h) && (layer->src_w != layer->dst_w) && (layer->index != 0)) {
			G2D_ERROR("Horizontal and vertical scaling at the same time can only be used with GPIPE0\n");
			return -1;
		}
	}
...
static int g2dlite_func_fill_rect_(void *g2d_regs, void *cmd_info, struct g2d_bg_cfg *bgcfg)
{
...
static int g2dlite_func_fill_rect(struct sdrv_g2d *gd,
	struct g2d_bg_cfg *bgcfg, struct g2d_output_cfg *output)
{
	int ret;

	ret = g2dlite_rdma_set(gd, NULL, true);
	if (ret) {
		G2D_ERROR("g2dlite rdma set failed\n");
		return -1;
	}

	ret = g2dlite_wdma_set(gd, output, false);
	if (ret) {
		G2D_ERROR("g2dlite wdma set failed\n");
		return -1;
	}

	g2dlite_mlc_clear(gd, 0);

	g2dlite_func_fill_rect_(gd->iomem_regs, &gd->cmd_info[0], bgcfg);

	return 0;
}

static void g2dlite_close_fastcopy(struct sdrv_g2d *gd)
{
	//g2dlite_r0p1_close_fastcopy(gd->iomem_regs);
	unsigned int val;
	...
	val = ap->ap_frm_ctrl;
	val = reg_value(0, val,
		AP_FRM_CTRL_FAST_CP_MODE_SHIFT, AP_FRM_CTRL_FAST_CP_MODE_MASK);
	ap->ap_frm_ctrl = val;
...
static int _g2dlite_fastcopy(void *g2d_regs, void *cmd_info, addr_t iaddr,
	uint32_t width, uint32_t height, addr_t oaddr, uint32_t stride)
{
	...
	REGISTER mlc_reg *mlc = (struct mlc_reg *)(g2d_regs + MLC_JUMP);
	cmdfile_info *cmd = (cmdfile_info *)cmd_info;
    ...
    /*MLC_BG_CTRL*/  /*Just for g2dlite rework*/
	val = mlc->mlc_bg_ctrl;
    val = reg_value(1, val, BG_A_SEL_SHIFT, BG_A_SEL_MASK);
    val = reg_value(1, val, BG_EN_SHIFT, BG_EN_MASK);
    g2d_cwrite(cmd, 0, MLC_BG_CTRL, val);
...
static int g2dlite_fastcopy(struct sdrv_g2d *gd, addr_t iaddr, u32 width,
	u32 height, u32 istride, addr_t oaddr, u32 ostride)
{
	...
	ret = g2dlite_wdma_set(gd, NULL, true);
	if (ret) {
		G2D_ERROR("g2dlite wdma set failed\n");
		return -1;
	}

	_g2dlite_fastcopy(gd->iomem_regs, &gd->cmd_info,
		iaddr, width, height, oaddr, ostride);

	return 0;
}

static int g2dlite_scaler_coef_set(struct sdrv_g2d *gd, struct g2d_coeff_table *tables)
{
	...
	if (!!tables->hcoef_set)
		g2dlite_set_hscaler_coef(gd, tables->hcoef);

	if (!!tables->vcoef_set) {
		g2dlite_set_vscaler_coef(gd, tables->vcoef);
	}
#if G2DLITE_DUMP_HVSCALER
	g2dlite_dump_hvscaler(gd);
#endif
...
const struct g2d_ops g2d_lite_ops = {
	.init = g2dlite_init,
	.enable = g2dlite_enable,
	.reset = g2dlite_reset,
	.mlc_set = g2dlite_mlc_set,
	.fill_rect = g2dlite_func_fill_rect,
	.fastcopy = g2dlite_fastcopy,
	.irq_handler = g2dlite_irq_handler,
	.rwdma = g2dlite_rwdma_set,
	.close_fastcopy = g2dlite_close_fastcopy,
	.wpipe_set = g2dlite_wpipe_set,
	.check_stroke = g2dlite_check_is_stroke,
	.scaler_coef_set = g2dlite_scaler_coef_set,
};
