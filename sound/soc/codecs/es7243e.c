...
struct snd_soc_codec *tron_codec[8];
...
static int
es7243e_set_bias_level(struct snd_soc_codec *codec,
...
static int es7243e_suspend(struct snd_soc_codec *codec)
{
	es7243e_set_bias_level(codec, SND_SOC_BIAS_OFF);
	return 0;
}

static int es7243e_resume(struct snd_soc_codec *codec)
{
	es7243e_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
	return 0;
}
...
static int es7243e_probe(struct snd_soc_codec *codec)
{
	struct es7243e_priv *es7243e = snd_soc_codec_get_drvdata(codec);
	int ret = 0;
...
#else
	codec->control_data = devm_regmap_init_i2c(es7243e->i2c,
	...
	tron_codec[es7243e_codec_num++] = codec;
	index = 0;
#if ES7243E_CHANNELS_MAX > 0
...
static int es7243e_remove(struct snd_soc_codec *codec)
{
	es7243e_set_bias_level(codec, SND_SOC_BIAS_OFF);
	return 0;
}

static struct snd_soc_codec_driver soc_codec_dev_es7243e = {
	.probe = es7243e_probe,
