...
/* defined for both unilink-i2s and rpmsg-i2s */
static struct snd_soc_dai_link snd_x9_ref_soc_dai_links[5] = {
	{
		.name = "x9_hifi_hs",
...
static struct snd_soc_dai_link snd_unilink_i2s_dai_links[] = {
	...
	{
		.name = "x9_unilink_pcm1",
		.stream_name = "x9 unilink pcm1",
		.cpu_name = "soc:unilink@i2s1",
		.cpu_dai_name = "snd-unilink-pcm-dai0",
		.platform_name = "soc:unilink@i2s1",
		.codec_name = "snd-soc-dummy",
		.codec_dai_name = "snd-soc-dummy-dai",
		.init = NULL,
	},
...
static int x9_ref_tlv320aic23_probe(struct platform_device *pdev)
{
	...
	int ret;
	int link_count = 1; /* default has one link */
	...
	if(cpu_np)
	{
		...
		snd_x9_ref_soc_dai_links[0].cpu_name = NULL;
		snd_x9_ref_soc_dai_links[0].platform_name = NULL;
		snd_x9_ref_soc_dai_links[0].cpu_of_node = cpu_np;
		snd_x9_ref_soc_dai_links[0].platform_of_node = cpu_np;
	}
	...
	if (of_find_property(dev->of_node, "semidrive,unilink_i2s", NULL)){
		dev_info(&pdev->dev, "add unilink_i2s device node\n");
		memcpy(&snd_x9_ref_soc_dai_links[link_count],&snd_unilink_i2s_dai_links[0],
	...
	if (of_find_property(dev->of_node, "semidrive,rpmsg_i2s", NULL)){
		...
		memcpy(&snd_x9_ref_soc_dai_links[link_count],&snd_rpmsg_i2s_dai_links[0],
				sizeof(snd_rpmsg_i2s_dai_links));
		link_count += ARRAY_SIZE(snd_rpmsg_i2s_dai_links);
	}

	x9_ref_tlv320aic23_card.num_links = link_count;
